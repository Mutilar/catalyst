...
class ProjectionBoundary extends Component<{ children: ReactNode; onFailure: (error: string) => void }, { error: string | null }> {
  ...
  render() {
    if (this.state.error) {
      return <div className="text-destructive whitespace-pre-wrap" role="alert">{this.state.error}</div>
    }
...
export function DirectOperation({ source }: { source: string }) {
  ...
  return (
    <div aria-busy={processing} className="w-full min-w-0 space-y-2" data-direct-operation="true" data-message-origin={preparation ? 'user-penguin' : 'user-operation'}>
      {preparation && <div className="text-xs font-medium text-muted-foreground">From user / PENGUIN</div>}
      {classifierGlyph && <div aria-label="Selected route" className="text-xs text-muted-foreground">{classifierGlyph}</div>}
      {processing && <div aria-live="polite" className="flex items-center gap-2">
      ...
      {error ? <div className="text-destructive" role="alert">{error}</div> : (
        <ProjectionBoundary key={source} onFailure={error => setFailure({ source, error })}>
          {gestalt !== null ? (
            ...
            <UguiTextContent allowContinuations={Boolean(onContinuation)} copyText={copyPayload} isRunning={processing}
              onContinuation={onContinuation} text={semanticSource} />
          ) : document && <McpUguiDocument copyText={copyPayload} document={document} onContinuation={onContinuation}
            presentationOnly />}
        </ProjectionBoundary>
      )}
    </div>
