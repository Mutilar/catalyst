...
| `AE_PENGUIN_TOOL_INTERPRETATION` | AE tool teaching mode: `off`, `heuristic` (default), or `intelligent`. Intelligent mode asks local PENGUIN for a candidate and deterministically validates it; failure falls back to the heuristic candidate. Suggestions never execute. `off` disables teaching only; categorical HARNESS prohibitions such as Git remain enforced. |
| `AE_PENGUIN_TOOL_HOLD_FOCUSED` | Set to `1` to hold focused Cargo test/build calls as teaching moments too. By default, focused diagnostic calls execute normally while root-wide/release calls are held. |
