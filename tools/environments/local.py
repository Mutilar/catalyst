...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    sanitized = _scope_delegated_child_env(sanitized)

    return sanitized


def _scope_delegated_child_env(env: dict[str, str]) -> dict[str, str]:
    """Carry delegated-child lineage into child subprocesses."""
    try:
        ...
        if is_delegated_child_process_context():
            return delegated_child_subprocess_env(env) or env
    except Exception:
        pass
    return env
...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 2 (conditional):** the rest of ``_HERMES_PROVIDER_ENV_BLOCKLIST``
    ...
    env = _scope_delegated_child_env(env)
...
def _make_run_env(env: dict) -> dict:
    ...
    run_env = _scope_delegated_child_env(run_env)
