...
def test_flush_persists_structured_api_content_sidecar(agent):
    ...
    db_write = agent._session_db.append_message.call_args.kwargs
    assert db_write["content"] == "Use this screenshot.\n[screenshot]"
    assert db_write["api_content"] == wire_content
...
class TestRunConversation:
    ...
    def test_redirect_during_thinking_retries_same_turn_with_context(self, agent):
        ...
        assert "original user request remains active" in checkpoint
        ...
        assert any(
            snapshot[-1].get("content", "").endswith("No, use Postgres instead.")
    ...
    @pytest.mark.parametrize("legacy_task", [None, "t_test_task_123"])
    def test_iteration_exhaustion_never_revives_retired_dispatcher(
        self, agent, monkeypatch, legacy_task
    ):
        self._setup_agent(agent)
        ...
        if legacy_task is None:
            ...
            monkeypatch.setenv("HERMES_KANBAN_TASK", legacy_task)
        ...
        retired_dispatcher = MagicMock()

        with (
            patch("run_agent.handle_function_call", return_value="ok"),
            patch.dict("sys.modules", {"hermes_cli.kanban_db": retired_dispatcher}),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
        ...
        assert retired_dispatcher.mock_calls == []
