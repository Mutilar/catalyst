...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_is_allocated_without_pid_aliasing(self):
        """mktemp remains unique when $$ is shared and BASHPID is unavailable."""
        env = _TestableEnv()
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
    ...
    def test_temp_path_template_is_quoted(self):
        """The same-directory mktemp template must preserve spaces as one word."""
        env = _TestableEnv()
        env._snapshot_ready = True
        env._snapshot_path = "/tmp/has space/hermes-snap-x.sh"
        wrapped = env._wrap_command("echo hi", "/tmp")
        assert "mktemp '/tmp/has space/hermes-snap-x.sh.tmp.XXXXXXXX'" in wrapped
    ...
    def test_init_session_bootstrap_also_atomic_and_unique(self):
        ...
        a uniquely allocated temporary file too."""
        env = _TestableEnv()
        captured = {}
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    that would corrupt PATH, including on Bash 3.2 without BASHPID.
    """
